// here we take fifo_depth =16 , addr_size =4 , data_size = 8 bits in this module 
...
//FIFO write pointer Handle module with full flag.
...
module wptr_full #(parameter ADDR_SIZE = 4)(
    ...
    input winc, wclk, wrst_n            // Write increment(write enable), write-clock, and reset
    );

    reg [ADDR_SIZE:0] wbin;                     // Binary write pointer(use to calculate next binary addres)
    ...
    assign wbin_next = wbin + (winc & ~wfull);       // Increment the write pointer if not full(overflow condition)
...
assign waddr = wbin[ADDR_SIZE-1:0];             // Write address calculation from the write pointer    
    ...
    always @(posedge wclk or negedge wrst_n) begin
        if (!wrst_n)            // Reset the full flag
            wfull <= 1'b0;       // i.e it has space to write 
        else 
...
module rptr_empty #(parameter ADDR_SIZE = 4)(
    ...
    input rinc, rclk, rrst_n            // Read increment(read enable), read-clock, and reset
    );

    reg [ADDR_SIZE:0] rbin;                     // Binary read pointer (used to calculte next read address )
    ...
    assign rbin_next = rbin + (rinc & ~rempty);         // Increment the read pointer if not empty(underflow condition)
...
module FIFO_memory #(parameter DATA_SIZE = 8,
    ...
    input [ADDR_SIZE-1:0] waddr, raddr,  // Write and read address // coming from write module and read module
    input wclk_en, wfull, wclk          // Write clock enable (from write module winc), write full, write clock
    );

    localparam DEPTH = 1<<ADDR_SIZE;     // Depth of the FIFO memory
    reg [DATA_SIZE-1:0] mem [0:DEPTH-1];// Memory array

    assign rdata = mem[raddr];          // Read data  // read is asynchronous and write is synchronous
...
module FIFO #(parameter DSIZE = 8,
    ...
    two_ff_sync #(ASIZE) sync_r2w (       // Read pointer syncronization to write clock domain
        .q2(wq2_rptr), 
        .din(rptr),
        .clk(wclk), 
        .rst_n(wrst_n)
    ...
    two_ff_sync #(ASIZE) sync_w2r (       // Write pointer syncronization to read clock domain
        .q2(rq2_wptr), 
        .din(wptr),
        .clk(rclk), 
        .rst_n(rrst_n)
